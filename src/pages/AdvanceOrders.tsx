...
import { CalendarDays, CheckCircle2, Clock3, Download, Eye, FileText, MessageCircle, PackageCheck, Plus, Printer, RefreshCw, Search, X, Trash2 } from 'lucide-react'
...
const inputClass = 'w-full rounded-xl border border-[#FDDBB4]/40 bg-white px-3.5 h-10 text-[14px] text-[#273126] outline-none transition focus:border-[#E87020] focus:ring-2 focus:ring-[#E87020]/20'
...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  const handleDelete = async (order: AdvanceOrder) => {
    const isCompleted = order.status === 'completed'
    const confirmMsg = isCompleted
      ? `⚠️ DELETE "${order.deposit_id}"?\n\nThis order is COMPLETED. Deleting it will:\n• Remove the advance order record\n• Remove the generated invoice (${order.invoice_number || 'N/A'})\n• Remove it from revenue analytics\n\nThis CANNOT be undone. Are you sure?`
    ...
    try {
      ...
      setOrders(rows => rows.filter(r => r.id !== order.id))
      if (selected?.id === order.id) setSelected(null)
      setNotice(`${order.deposit_id} has been permanently deleted.${isCompleted ? ' Invoice and analytics entry removed.' : ''}`)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to delete advance order')
    }
  }
  ...
  const StatusSelect = (order: AdvanceOrder, className = '') => <select value={order.status} onChange={e => void changeStatus(order, e.target.value as AdvanceStatus)} className={`rounded-lg border px-2 py-1.5 text-xs font-black ${STATUS_STYLES[order.status]} ${className}`}><option value="pending_deposit">Pending Deposit</option><option value="waiting_final_payment">Waiting for Final Payment</option><option value="completed">Completed (receive payment)</option><option value="ready_for_delivery">Ready to Collect</option><option value="cancelled">Cancelled</option></select>

  const OrderActions = (order: AdvanceOrder) => <div className="flex flex-wrap gap-1.5"><button onClick={() => void openDetails(order)} className="rounded-lg bg-[#F4F2F6] p-2 text-violet-700" title="Details"><Eye size={16}/></button>{order.status !== 'completed' && order.status !== 'cancelled' && <button onClick={() => setPaymentOrder(order)} className="rounded-lg bg-emerald-600 px-2.5 py-2 text-[11px] font-black text-white">Receive Balance</button>}{order.status !== 'completed' && <><button onClick={() => printAdvanceReceipt(order)} className="rounded-lg bg-amber-50 p-2 text-amber-700" title="Print advance receipt"><Printer size={16}/></button><button onClick={() => downloadFile(advanceReceiptPdf(order))} className="rounded-lg bg-amber-50 p-2 text-amber-700" title="Download advance receipt"><Download size={16}/></button><button onClick={() => whatsappDepositReceipt(order)} className="rounded-lg bg-emerald-50 p-2 text-emerald-700" title="WhatsApp advance receipt"><MessageCircle size={16}/></button></>}{order.status === 'completed' && <><button onClick={() => downloadFile(invoiceFile(order))} className="rounded-lg bg-violet-50 p-2 text-violet-700" title="PDF invoice"><Download size={16}/></button><button onClick={() => printFinal(order)} className="rounded-lg bg-violet-50 p-2 text-violet-700" title="Print receipt"><Printer size={16}/></button><button onClick={() => whatsappInvoice(order)} className="rounded-lg bg-emerald-50 p-2 text-emerald-700" title="WhatsApp invoice"><MessageCircle size={16}/></button></>}{role === 'admin' && <button onClick={() => void handleDelete(order)} className="rounded-lg bg-red-50 p-2 text-red-500 hover:bg-red-100 hover:text-red-700" title="Delete advance order"><Trash2 size={16}/></button>}</div>

  return <div className="space-y-4 md:space-y-5 px-4 py-4 md:px-0 md:py-0">
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3"><div><p className="text-[11px] md:text-xs font-black uppercase tracking-[.18em] text-[#E87020]">Separate from sales</p><h2 className="text-[16px] md:text-2xl font-black text-[#273126]">Advance Orders</h2><p className="mt-1 text-[12px] md:text-sm text-[#6B7280]">Deposits never count as revenue. Full order value is recognized only after final payment.</p></div><div className="flex w-full md:w-auto gap-2"><button onClick={() => void load()} className="rounded-xl border border-[#FDDBB4]/40 bg-white h-10 w-10 flex items-center justify-center text-[#647064]" title="Refresh"><RefreshCw size={18}/></button><button onClick={() => setCreateOpen(true)} className="flex-1 md:flex-none flex items-center justify-center gap-2 rounded-xl bg-[#E87020] h-10 px-4 text-[14px] font-black text-white hover:bg-[#d66318]"><Plus size={18}/><span className="hidden md:inline">New Advance Order</span><span className="md:hidden">New Order</span></button></div></div>
    {error && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-[14px] font-semibold text-red-700">{error}</div>}
    {notice && <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-[14px] font-semibold text-emerald-700">{notice}</div>}
    <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 xl:grid-cols-3">{cards.map(([label, value, Icon, color]) => <div key={label} className="rounded-2xl border border-[#FDDBB4]/40 bg-white p-3 md:p-4 shadow-sm"><div className="flex items-center justify-between"><div><p className="text-[11px] font-black uppercase tracking-wide text-[#879086]">{label}</p><p className="mt-2 text-[20px] md:text-2xl font-black text-[#273126]">{value}</p></div><div className={`rounded-xl p-3 ${color}`}><Icon size={21}/></div></div></div>)}</div>
    <div className="rounded-2xl border border-[#FDDBB4]/40 bg-white p-3 md:p-4 shadow-sm"><div className="grid gap-3 grid-cols-1 lg:grid-cols-[1fr_auto_auto]"><label className="relative flex items-center"><Search className="absolute left-3 text-[#9CA3AF]" size={17}/><input className={`${inputClass} pl-10`} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Deposit ID, customer..."/></label><div className="flex flex-wrap gap-2">{(['all','pending','ready','completed','cancelled'] as StatusFilter[]).map(value => <button key={value} onClick={() => setStatusFilter(value)} className={`rounded-lg px-3 h-10 text-[11px] font-black capitalize ${statusFilter === value ? 'bg-[#E87020] text-white' : 'bg-[#F5F3F7] text-[#626B61]'}`}>{value}</button>)}</div><select className={inputClass} value={dateFilter} onChange={e => setDateFilter(e.target.value as DateFilter)}><option value="all">All Dates</option><option value="today">Today</option><option value="week">This Week</option><option value="month">This Month</option></select></div></div>
    <div className="md:hidden rounded-xl border border-[#ECE9E2] divide-y divide-[#ECE9E2] overflow-hidden bg-white shadow-sm">{loading ? <div className="p-6 text-center text-[12px] text-[#6B7280]">Loading advance orders...</div> : filtered.length === 0 ? <div className="p-6 text-center text-[12px] text-[#6B7280]">No advance orders match these filters.</div> : filtered.map(order => <div key={order.id} className="p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-[12px] font-black text-violet-700">{order.deposit_id}</p><p className="text-[10px] text-[#8B9389]">{new Date(order.created_at).toLocaleDateString('en-MY')} - {new Date(order.created_at).toLocaleTimeString('en-MY',{hour:'2-digit',minute:'2-digit'})}</p></div>{StatusSelect(order, '!text-[10px] !px-1.5 !py-1 shrink-0 max-w-[110px]')}</div><div className="mt-2 grid grid-cols-2 gap-2 text-[11px]"><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-wide text-[#9BAB9A]">Customer</p><p className="truncate font-bold text-[#273126]">{order.customer_name}</p><p className="truncate text-[10px] text-[#727970]">{order.phone}</p></div><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-wide text-[#9BAB9A]">Product</p><p className="truncate font-semibold">{order.product_name}</p><p className="truncate text-[10px] text-[#858C83]">{order.category || 'Uncategorised'}</p></div><div><p className="text-[9px] font-black uppercase tracking-wide text-[#9BAB9A]">Total / Paid / Balance</p><p>Total <b>{formatCurrency(order.total_amount)}</b></p><p className="text-violet-700">Paid <b>{formatCurrency(order.deposit_amount)}</b></p><p className="text-red-600">Balance <b>{formatCurrency(order.remaining_balance)}</b></p></div><div><p className="text-[9px] font-black uppercase tracking-wide text-[#9BAB9A]">Delivery</p><div className="mt-0.5 flex items-center gap-1"><CalendarDays size={12}/>{new Date(`${order.expected_delivery_date}T00:00:00`).toLocaleDateString('en-MY')}</div></div></div><div className="mt-2.5 border-t border-[#F0EEE9] pt-2.5 [&_button]:!p-1.5 [&_button]:!text-[10px] [&_svg]:!w-3.5 [&_svg]:!h-3.5">{OrderActions(order)}</div></div>)}</div>
    <div className="hidden overflow-hidden rounded-2xl border border-[#ECE9E2] bg-white shadow-sm md:block"><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-[#F8F7F4] text-[10px] font-black uppercase tracking-wide text-[#737B72]"><tr>{['Deposit ID / Created','Customer','Product','Total / Deposit / Balance','Delivery','Status','Actions'].map(h => <th key={h} className="px-4 py-3">{h}</th>)}</tr></thead><tbody className="divide-y divide-[#F0EEE9]">{loading ? <tr><td colSpan={7} className="px-4 py-12 text-center text-[#6B7280]">Loading advance orders...</td></tr> : filtered.length === 0 ? <tr><td colSpan={7} className="px-4 py-12 text-center text-[#6B7280]">No advance orders match these filters.</td></tr> : filtered.map(order => <tr key={order.id} className="hover:bg-violet-50/30"><td className="px-4 py-3"><p className="font-black text-violet-700">{order.deposit_id}</p><p className="text-[11px] text-[#8B9389]">{new Date(order.created_at).toLocaleDateString('en-MY')} - {new Date(order.created_at).toLocaleTimeString('en-MY',{hour:'2-digit',minute:'2-digit'})}</p></td><td className="px-4 py-3"><p className="font-bold text-[#273126]">{order.customer_name}</p><p className="text-xs text-[#727970]">{order.phone}</p></td><td className="max-w-[190px] px-4 py-3"><p className="truncate font-semibold">{order.product_name}</p><p className="truncate text-xs text-[#858C83]">{order.category || 'Uncategorised'}</p></td><td className="px-4 py-3 text-xs"><p>Total <b>{formatCurrency(order.total_amount)}</b></p><p className="text-violet-700">Paid <b>{formatCurrency(order.deposit_amount)}</b></p><p className="text-red-600">Balance <b>{formatCurrency(order.remaining_balance)}</b></p></td><td className="px-4 py-3"><div className="flex items-center gap-1.5"><CalendarDays size={14}/>{new Date(`${order.expected_delivery_date}T00:00:00`).toLocaleDateString('en-MY')}</div></td><td className="px-4 py-3">{StatusSelect(order)}</td><td className="px-4 py-3">{OrderActions(order)}</td></tr>)}</tbody></table></div></div>

    {createOpen && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/55 p-4"><form onSubmit={create} className="max-h-[94vh] w-full max-w-4xl overflow-hidden overflow-y-auto rounded-3xl bg-white p-4 md:p-6 shadow-2xl"><div className="mb-5 flex items-center justify-between"><div><h3 className="text-xl font-black">Create Advance Order</h3><p className="text-[12px] text-[#879086]">Creates an advance receipt only - no revenue or final invoice.</p></div><button type="button" onClick={() => setCreateOpen(false)}><X/></button></div><div className="grid gap-3 md:gap-4 md:grid-cols-2"><Field label="Customer Name *"><input required className={inputClass} value={form.customerName} onChange={e=>setForm({...form,customerName:e.target.value})}/></Field><Field label="Phone Number *"><input required className={inputClass} value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/></Field><Field label="Address"><textarea className={inputClass} value={form.address} onChange={e=>setForm({...form,address:e.target.value})}/></Field><Field label="Product Name *"><input required list="advance-products" className={inputClass} value={form.productName} onChange={e=>{const product=products.find(p=>p.name===e.target.value);setForm({...form,productName:e.target.value,category:product?.category||form.category})}}/><datalist id="advance-products">{products.map(p=><option key={p.id} value={p.name}/>)}</datalist></Field><Field label="Category"><input className={inputClass} value={form.category} onChange={e=>setForm({...form,category:e.target.value})}/></Field><Field label="Description"><textarea className={inputClass} value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></Field><Field label="Total Order Amount *"><input required min="0.01" step="0.01" type="number" className={inputClass} value={form.totalAmount} onChange={e=>setForm({...form,totalAmount:e.target.value})}/></Field><Field label="Deposit Amount Received *"><input required min="0" step="0.01" type="number" className={inputClass} value={form.depositAmount} onChange={e=>setForm({...form,depositAmount:e.target.value})}/></Field><Field label="Remaining Balance (automatic)"><div className="rounded-xl bg-[#FDDBB4]/20 px-4 py-2 h-10 flex items-center font-black text-[#E87020]">{formatCurrency(Math.max(0, Number(form.totalAmount||0)-Number(form.depositAmount||0)))}</div></Field><Field label="Deposit Payment Method"><select className={inputClass} value={form.paymentMethod} onChange={e=>setForm({...form,paymentMethod:e.target.value})}><option value="">Select Method</option>{PAYMENT_METHODS.map(m=><option key={m} value={m}>{m}</option>)}</select></Field><Field label="Expected Delivery Date *"><input required type="date" className={inputClass} value={form.expectedDeliveryDate} onChange={e=>setForm({...form,expectedDeliveryDate:e.target.value})}/></Field><Field label="Order Status"><select disabled className={inputClass} value="pending_deposit"><option value="pending_deposit">Pending Deposit</option></select></Field>      <div className="md:col-span-2"><Field label="Reference Number"><input className={inputClass} value={form.reference_number} onChange={e=>setForm({...form,reference_number:e.target.value})} placeholder="e.g. PO-001, booking ref (optional)"/></Field></div><div className="md:col-span-2"><Field label="Remarks"><textarea className={inputClass} value={form.remarks} onChange={e=>setForm({...form,remarks:e.target.value})} placeholder="e.g. special instructions, colour, size notes"/></Field></div></div><div className="mt-6 flex flex-col md:flex-row justify-end gap-3"><button type="button" onClick={()=>setCreateOpen(false)} className="rounded-xl border h-10 px-5 text-[14px] font-bold w-full md:w-auto">Cancel</button><button disabled={saving} className="rounded-xl bg-[#E87020] h-10 px-5 text-[14px] font-black text-white disabled:opacity-50 w-full md:w-auto">{saving?'Creating...':'Create & Save Advance Receipt'}</button></div></form></div>}
