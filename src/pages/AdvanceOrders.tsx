...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  const handleDelete = async (order: AdvanceOrder) => {
    ...
    if (isCompleted) {
      ...
      if (input?.trim().toUpperCase() !== 'DELETE') { setError('Deletion cancelled — you must type DELETE to confirm.'); return }
    } else {
